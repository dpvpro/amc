...
// run executes the reflector pipeline and returns the exit code.
func run(argv []string) int {
	...
	if cfg != "" {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "reflector: cannot read config %s: %v\n", cfg, err)
			return 1
		}
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "reflector: %v\n", err)
		return 2
	}
	if opts.Sort != "" && !contains(sortChoices, opts.Sort) {
		fmt.Fprintf(os.Stderr, "reflector: invalid sort criterion %q (choose from %v)\n", opts.Sort, sortChoices)
		return 2
	}
	...
	if opts.Info {
		if len(mirrors) == 0 {
			fmt.Fprintln(os.Stderr, "reflector: no mirrors found")
			return 1
		}
